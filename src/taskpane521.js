...
function _brandBodyHasSupportingSignals(config, bodyLower, subjectLower, senderDomainLower, bodyOriginal) {
    ...
    for (const keyword of config.keywords) {
        ...
        if (totalMentions >= 3 && liveMentions >= 2) {
            // v5.2.1: Require conversion vector (external link or formatted phone number)
            // for mention_count. Prevents firing on business discussions about products.
            // Phone regex requires separators between digit groups to reject order/invoice numbers.
            const hasExternalLink = /https?:\/\/[^\s<>"')\]]+/.test(bodyOriginal || '');
            const hasPhoneNumber = /(?:\+\d{1,3}[\s.-])?\(?\d{2,4}\)?[\s.-]\d{3,4}[\s.-]\d{4}\b/.test(bodyOriginal || '');
