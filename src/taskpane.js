...
const QR_CONFIG = {
    ...
    maxDimension: 600,      // v6.2.0: Skip images larger than 600px (hero banners, photos)
...
async function decodeQRCodesFromHtml(htmlString) {
    ...
    try {
        ...
        for (const imgEl of images) {
            ...
            try {
                const url = await decodeQRFromImageSrc(src);
                if (url) {
                    decoded.push(url);
                    console.log('[EFA QR v6.2.0] Decoded URL from QR code:', url);
                }
            } catch (e) {
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            try {
                ...
                if (w < QR_CONFIG.minDimension || h < QR_CONFIG.minDimension) { resolve(null); return; }
                if (w > QR_CONFIG.maxDimension || h > QR_CONFIG.maxDimension) { resolve(null); return; }

                // v6.2.0: Aspect ratio filter — QR codes are square
                const ratio = w / h;
                if (ratio < QR_CONFIG.aspectRatioMin || ratio > QR_CONFIG.aspectRatioMax) { resolve(null); return; }
