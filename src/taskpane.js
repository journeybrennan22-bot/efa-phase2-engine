...
function evaluatePatternB(signals, coreWarnings, preGateBrandResult) {
    ...
    // Pattern B should still fire if a body brand was detected pre-gate AND Pattern B's
    // own strong signals (suspicious hosting link + credential/urgency) are present.
    // This prevents Viktor's attack: clean domain + body brand + Firebase credential harvester.
    ...
    if (suspiciousHostingLink) {
        ...
        if (credentialLanguage || hasUrgency) {
            return {
                ...
                confidence: 'high',
                signals: { brand_mismatch: brandSource, suspicious_hosting_link: suspiciousHostingLink, credential_language: credentialLanguage || null, urgency: hasUrgency || null },
    ...
    if (legitPlatformLink) {
        ...
        if (supportingSignalCount >= 2) {
            return {
                ...
                patternName: 'Brand Impersonation with Suspicious Link',
                confidence: 'medium',
                signals: { brand_mismatch: brandSource, legit_platform_link: legitPlatformLink, supporting_signal_count: supportingSignalCount },
