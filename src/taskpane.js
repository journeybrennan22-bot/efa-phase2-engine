...
async function processEmail(emailData) {
    ...
    if (disposableEmail) {
        warnings.push({
            ...
            description: `This email was sent from ${disposableEmail.provider}, a disposable email service that requires no identity verification. These temporary inboxes are used to send untraceable messages. <b>No legitimate</b> business correspondent uses throwaway email.`,
            senderEmail: senderEmail,
    ...
    // suppression filter cannot wipe it. Intentionally unfilterable.
    {
        ...
        if (matchedPaymentPhrases.length > 0) {
            ...
            warnings.push({
                ...
                description: 'This email requests a change to payment or wire instructions. This is the #1 method used in wire fraud — even when the sender appears legitimate. <b>ALWAYS verify payment changes by phone</b> using a number you already have on file.',
                senderEmail: senderEmail,
