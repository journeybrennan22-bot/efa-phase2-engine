...
function detectBrandImpersonation(subject, body, senderDomain, displayName) {
    ...
    for (const [brandName, config] of Object.entries(BRAND_CONTENT_DETECTION)) {
        ...
        if (mentionsBrand) {
            ...
            if (!isLegitimate) {
                // v5.1.0: Skip when sender is a known ESP (Brevo, SendGrid, Mailchimp, etc.)
                // ESPs send on behalf of businesses - brand mentions in the body/subject are expected.
                // "5691434.brevosend.com" mentioning Dnb Bank = Brevo sending Dnb Bank's newsletter.
                // Viktor's ESP phishing is caught by reply-to freemail check + wire fraud keywords.
                ...
                if (isESPDomain) {
                    ...
                    continue; // Skip this brand, check next brand in loop
                }
...
function detectOrganizationImpersonation(displayName, senderDomain) {
    ...
    // v5.1.0: Skip when sender is a known ESP. ESPs send on behalf of organizations,
    // so display names containing org names are expected (e.g., "Orange" via brevosend.com).
