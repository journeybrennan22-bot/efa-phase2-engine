...
const ESP_SENDER_DOMAINS = new Set([
    ...
    'qemailserver.com',
    // v6.0.1: Whiteboard Mortgage CRM sending domain (verified via support.whiteboard.software)
    'whiteboard.email'
]);
...
const PROTECTED_COMMON_WORD_BRANDS = new Set([
    ...
    'nationwide', 'progressive', 'prudential', 'guardian', 'wish'
]);
...
// ============================================
// v6.2.0: QR CODE PHISHING DETECTION ("QUISHING")
// Decodes QR codes from inline email images and extracts embedded URLs.
// Decoded URLs are fed into the existing analysis pipeline (Phase 1 + Phase 2).
// Viktor: "The QR code is how I get the mailbox. Decode it and you cut me off at the root."
// ============================================
const QR_CONFIG = {
    maxImages: 5,           // v6.2.0: Cap images to decode per email
    minDimension: 100,      // v6.2.0: Skip images smaller than 100px (tracking pixels, icons)
    maxDimension: 1000,     // v6.2.0: Skip images larger than 1000px. Viktor: QR campaigns render at 700-800px.
    aspectRatioMin: 0.8,    // v6.2.0: QR codes are square — skip non-square images
    aspectRatioMax: 1.2,
    decodeTimeoutMs: 500    // v6.2.0: Per-image timeout to prevent hangs from malformed QR
};

// v6.2.0: Decode QR codes from HTML email body (Outlook).
// Parses <img> tags, draws to offscreen canvas, runs jsQR, extracts URLs.
// Returns array of decoded URL strings. Fails silently on CORS/load errors.
// Viktor-test: try/catch + timeout on every image. Malformed QR can't hang the scanner.
async function decodeQRCodesFromHtml(htmlString) {
    const decoded = [];
    if (typeof jsQR !== 'function') {
        console.log('[EFA QR v6.2.0] jsQR library not loaded, skipping QR decode');
        return decoded;
    }
    if (!htmlString) return decoded;
    ...
    try {
        ...
        for (const imgEl of images) {
            if (processed >= QR_CONFIG.maxImages) break;
            const src = imgEl.src || imgEl.getAttribute('src') || '';
            if (!src) continue;

            try {
                const result = await decodeQRFromImageSrc(src);
                if (result === 'filtered') continue; // v6.2.0 Viktor fix: filtered images don't count toward cap
                if (result) {
                    decoded.push(result);
                    console.log('[EFA QR v6.2.0] Decoded URL from QR code:', result);
                }
            } catch (e) {
                console.log('[EFA QR v6.2.0] Image decode error:', e.message);
            }
            processed++; // Only counts images that attempted actual QR decode
        }
    } catch (e) {
        console.log('[EFA QR v6.2.0] HTML parsing error:', e.message);
    }

    return decoded;
}

// v6.2.0: Load a single image src, draw to canvas, attempt jsQR decode.
// Returns URL string if QR code found with http(s) URL, null otherwise.
// Viktor-test: 500ms timeout per image. CORS errors caught and skipped.
function decodeQRFromImageSrc(src) {
    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            console.log('[EFA QR v6.2.0] Timeout decoding image, skipping');
            resolve(null);
        }, QR_CONFIG.decodeTimeoutMs);
        ...
        img.onload = () => {
            ...
            try {
                ...
                // v6.2.0: Size filter — skip tracking pixels and hero banners
                if (w < QR_CONFIG.minDimension || h < QR_CONFIG.minDimension) { resolve('filtered'); return; }
                if (w > QR_CONFIG.maxDimension || h > QR_CONFIG.maxDimension) { resolve('filtered'); return; }

                // v6.2.0: Aspect ratio filter — QR codes are square
                const ratio = w / h;
                if (ratio < QR_CONFIG.aspectRatioMin || ratio > QR_CONFIG.aspectRatioMax) { resolve('filtered'); return; }

                // v6.2.0: Draw to offscreen canvas and extract pixel data
                ...
                // v6.2.0: Run jsQR decoder
                const qrResult = jsQR(imageData.data, w, h);
                if (qrResult && qrResult.data) {
                    const data = qrResult.data.trim();
                    // Viktor-test: Only treat as URL if starts with http:// or https://.
                    // Discard vCard, Wi-Fi, plain text, etc. Can't be used for phishing without a URL.
                ...
                // v6.2.0: Canvas tainted by CORS or other error — skip silently
                console.log('[EFA QR v6.2.0] Canvas error:', e.message);
                resolve(null);
            }
        };
        ...
        img.onerror = () => {
            clearTimeout(timeout);
            resolve(null); // v6.2.0: Image load failure — skip silently
        };
...
// v6.2.0: Analyze decoded QR URLs against existing detection pipeline.
// Runs lookalike, suspicious domain, free hosting, fake TLD, brand mismatch, and reputation checks.
// Returns array of warning objects (type: 'qr-phishing') for any flagged URLs.
// Viktor-test: No trust bypass. Compromised trusted mailbox sending QR phishing is the VEC play.
async function analyzeQRUrls(qrUrls, senderDomain, senderEmail, brandResult) {
    const qrWarnings = [];
    for (const qrUrl of qrUrls) {
        let qrDomain = '';
        try { qrDomain = new URL(qrUrl).hostname.toLowerCase(); } catch (e) { continue; }

        const qrFlags = []; // v6.2.0: Collect all reasons this QR URL is suspicious

        // v6.2.0: Lookalike domain check (Levenshtein)
        const lookalike = detectLookalikeDomain(qrDomain);
        if (lookalike) qrFlags.push('lookalike of ' + (lookalike.trustedDomain || lookalike));

        // v6.2.0: Suspicious domain check (high-risk TLDs, fake country-code combos)
        const suspicious = detectSuspiciousDomain(qrDomain);
        if (suspicious) qrFlags.push(suspicious.reason || 'suspicious domain characteristics');

        // v6.2.0: Free hosting platform check
        const freeHosting = detectFreeHostingSender(qrDomain);
        if (freeHosting) qrFlags.push('free hosting platform: ' + freeHosting.hostingPlatform);

        // v6.2.0: Fake TLD check
        const fakeTld = detectFakeTLD(qrDomain);
        if (fakeTld) qrFlags.push('fake domain extension: ' + fakeTld.fakeTLD);

        // v6.2.0: Brand mismatch — QR URL goes to domain that doesn't match claimed brand
        let isBrandMismatch = false;
        if (brandResult) {
            const isLegitBrandUrl = brandResult.legitimateDomains.some(ld =>
                qrDomain === ld || qrDomain.endsWith('.' + ld));
            if (!isLegitBrandUrl) {
                qrFlags.push('QR link does not match claimed brand ' + brandResult.brandName);
                isBrandMismatch = true;
            }
        }

        // v6.2.0: Sender-link mismatch — QR URL goes to unrelated domain
        if (qrDomain !== senderDomain && !qrDomain.endsWith('.' + senderDomain) && !senderDomain.endsWith('.' + qrDomain)) {
            const isKnownBrandUrl = isOfficialBrandDomain(qrDomain);
            const isKnownPlatformUrl = isKnownPlatform(qrDomain);
            if (!isKnownBrandUrl && !isKnownPlatformUrl) {
                qrFlags.push('links to unrelated domain');
            }
        }

        // v6.2.0: Domain reputation check (threat feeds + WHOIS age via Cloudflare Worker)
        try {
            const qrReputation = await checkDomainReputation(qrDomain);
            if (qrReputation) {
                if (qrReputation.layer === 'threat_feed') qrFlags.push('known malicious domain');
                if (qrReputation.layer === 'whois_age') qrFlags.push('newly registered domain (' + qrReputation.domainAge + ' days old)');
            }
        } catch (e) {
            console.log('[EFA QR v6.2.0] Reputation check failed for', qrDomain, ':', e.message);
        }

        if (qrFlags.length > 0) {
            const hasCriticalFlag = !!(lookalike || fakeTld || isBrandMismatch || freeHosting);
            qrWarnings.push({
                type: 'qr-phishing',
                severity: hasCriticalFlag ? 'critical' : 'high',
                title: 'QR Code Contains Suspicious Link',
                description: 'This email contains a QR code that links to ' + qrDomain + '. Scanning this code would take you to a potentially dangerous website. Do not scan QR codes from unexpected emails.',
                senderEmail: senderEmail,
                qrUrl: qrUrl,
                qrDomain: qrDomain,
                qrFlags: qrFlags
            });
            console.log('[EFA QR v6.2.0] FLAGGED QR URL:', qrUrl, '| Flags:', qrFlags.join(', '));
        }
    }
    return qrWarnings;
}
...
const PHISHING_URGENCY_KEYWORDS = [
    ...
    // v5.2.2: Transactional/confirmation-style phishing (Viktor Tier-1b gap analysis)
    'payment received', 'payment processed', 'payment confirmation',
    'order confirmation', 'order placed', 'order shipped',
    'purchase confirmation', 'transaction confirmation',
    'subscription renewed', 'plan upgraded', 'plan changed',
    'receipt for your', 'receipt from',
    'invoice for', 'invoice from',
    'unauthorized transaction', 'unauthorized purchase', 'unauthorized charge',
    'unrecognized transaction', 'unrecognized charge',
    'did you authorize', 'you sent a payment', 'you made a purchase'
];
...
const BRAND_CONTENT_DETECTION = {
    ...
    'microsoft': {
        ...
        legitimateDomains: ['microsoft.com', 'office.com', 'sharepoint.com', 'onedrive.com', 'live.com', 'outlook.com', 'office365.com', 'teams.mail.microsoft', 'microsoft365.com', 'microsoftsupport.com'] // v6.0.2: added microsoft365.com (rebrand sending domain, live FP) + microsoftsupport.com (Microsoft Learn docs)
    },
    'google': {
    ...
    'amazon': {
        ...
        legitimateDomains: ['amazon.com', 'amazon.co.uk', 'amazon.ca', 'amazonses.com', 'amazon.de', 'amazon.fr', 'amazon.it', 'amazon.es', 'amazon.co.jp', 'amazon.in', 'amazon.com.au', 'amazon.com.br', 'amazon.com.mx', 'amazon.nl', 'amazon.pl', 'amazon.sg', 'amazon.ae', 'amazon.sa', 'amazon.se', 'amazon.eg', 'amazon.cn', 'amazon.com.be', 'amazon.com.tr'] // v6.0.2: added all active Amazon regional country domains
    },
    'paypal': {
        keywords: ['paypal'],
    ...
    'apple': {
        ...
        legitimateDomains: ['apple.com', 'icloud.com', 'itunes.com'] // v6.0.2: added itunes.com (Apple Community docs confirm as official sender domain)
    },
    'facebook': {
    ...
    'yahoo': {
        ...
        legitimateDomains: ['yahoo.com', 'yahoomail.com', 'comms.yahoo.net', 'ymail.com', 'myyahoo.com', 'rocketmail.com'] // v6.0.2: COP-approved Yahoo infra + legacy account domains
    },
    'mcafee': {
        keywords: ['mcafee'],
    ...
    'dhl': {
        ...
        legitimateDomains: ['dhl.com', 'dhl.de', 'dpdhl.com', 'dhl-news.com', 'dhl.fr', 'dhl.co.uk', 'dhl.it', 'dhl.es', 'dhl.nl', 'dhl.at', 'dhl.ch', 'dhl.be', 'dhl.se', 'dhl.pl', 'dhl.co.jp', 'dhl.com.au', 'dhl.com.br', 'dhl.com.sg', 'dhl.co.in'] // v6.0.2: dpdhl.com + dhl-news.com per DHL fraud awareness page, plus major regional country domains
    },
    'fedex': {
    ...
    'quickbooks': {
        ...
        legitimateDomains: ['intuit.com', 'quickbooks.com', 'tsheets.com', 'macpayroll.com'] // v6.0.2: added tsheets.com (QB Time) + macpayroll.com (QB Mac Payroll) per Intuit official service email docs
    },
    'zoom': {
    ...
    'ebay': {
        keywords: ['ebay'],
        legitimateDomains: ['ebay.com', 'ebay.co.uk', 'ebay.de', 'ebay.fr', 'ebay.it', 'ebay.es', 'ebay.com.au', 'ebay.ca', 'ebay.nl', 'ebay.pl', 'ebay.ie', 'ebay.at', 'ebay.ch', 'ebay.com.sg'] // v6.0.2: added all active eBay regional sites per eBay international fees page
    },
    'dmv': {
    ...
    'xbox': {
        ...
        legitimateDomains: ['xbox.com', 'microsoft.com', 'microsoft365.com'] // v6.0.2: added microsoft365.com (rebrand domain)
    },
    'epic games': {
    ...
    'disney plus': {
        ...
        legitimateDomains: ['disneyplus.com', 'disney.com', 'go.com', 'd23.com', 'disneyonline.com', 'disneyaccount.com']
    },
    'hulu': {
    ...
    'macys': {
        keywords: ['macys order', 'macys account', 'macys card', 'macys credit', 'macy\'s order', 'macy\'s account', 'macy\'s card'],
    ...
    'deutsche post': {
        ...
        legitimateDomains: ['deutschepost.de', 'dhl.de', 'dpdhl.com'] // v6.0.2: added dpdhl.com (parent company domain)
    },
    'la poste': {
    ...
    'disney': { keywords: ['disney', 'walt disney', 'disneyland', 'disney world'], legitimateDomains: ['disney.com', 'disneyplus.com', 'go.com', 'disneyland.disney.go.com', 'disneyworld.disney.go.com', 'd23.com', 'thewaltdisneycompany.com', 'disneyonline.com', 'disneyaccount.com'] },
    ...
    'kindle': { keywords: ['kindle unlimited'], legitimateDomains: ['amazon.com', 'kindle.com', 'amazon.co.uk', 'amazon.de', 'amazon.fr', 'amazon.it', 'amazon.es', 'amazon.co.jp', 'amazon.in', 'amazon.com.au'] }, // v6.0.2: added major regional Amazon domains for Kindle notifications
    ...
    'wish': { keywords: ['wish app', 'wish order', 'wish account', 'wish shopping', 'wish marketplace', 'wish.com'], legitimateDomains: ['wish.com'] },
    ...
    'intuit': { keywords: ['intuit'], legitimateDomains: ['intuit.com', 'tsheets.com', 'macpayroll.com'] }, // v6.0.2: added tsheets.com + macpayroll.com
    ...
    // v6.1.0: Identity providers. AiTM kits impersonate these to steal session tokens.
    // Viktor: "I don't need your password. I need your session cookie after you log in."
    'okta': {
        keywords: ['okta', 'okta account', 'okta session', 'okta verify', 'okta sign-in'],
        legitimateDomains: ['okta.com', 'okta-emea.com']
    },
    'auth0': {
        keywords: ['auth0', 'auth0 account'],
        legitimateDomains: ['auth0.com']
    },
    'onelogin': {
        keywords: ['onelogin', 'onelogin account', 'onelogin session'],
        legitimateDomains: ['onelogin.com']
    },
    'duo security': {
        keywords: ['duo security', 'duo mobile', 'duo push', 'duo authentication'],
        legitimateDomains: ['duosecurity.com', 'duo.com']
    },
    'ping identity': {
        keywords: ['ping identity', 'pingone', 'pingfederate'],
        legitimateDomains: ['pingidentity.com']
    },
    'microsoft entra': {
        keywords: ['microsoft entra', 'entra id', 'azure ad', 'azure active directory'],
        legitimateDomains: ['microsoft.com', 'microsoftonline.com']
    }
};
...
const IMPERSONATION_TARGETS = {
    ...
    "dhl": ["dhl.com", "dhl.de", "dpdhl.com", "dhl-news.com", "dhl.fr", "dhl.co.uk", "dhl.it", "dhl.es", "dhl.nl", "dhl.at", "dhl.ch", "dhl.be", "dhl.se", "dhl.pl", "dhl.co.jp", "dhl.com.au", "dhl.com.br", "dhl.com.sg", "dhl.co.in"], // v6.0.2: added regional + infra domains
    ...
    "usaa": ["usaa.com"],
    "microsoft support": ["microsoft.com", "microsoft365.com", "microsoftsupport.com"], // v6.0.2: added rebrand + support domains
    "microsoft account": ["microsoft.com", "live.com", "microsoft365.com"], // v6.0.2: added microsoft365.com
    "microsoft security": ["microsoft.com", "microsoft365.com"], // v6.0.2: added microsoft365.com
    "apple support": ["apple.com", "icloud.com", "itunes.com"], // v6.0.2: added icloud.com + itunes.com
    "apple id": ["apple.com", "icloud.com", "itunes.com"], // v6.0.2: added icloud.com + itunes.com
    "apple security": ["apple.com", "icloud.com"], // v6.0.2: added icloud.com
    "google support": ["google.com"],
    "google account": ["google.com"],
    "google security": ["google.com"],
    "amazon support": ["amazon.com", "amazon.co.uk", "amazon.ca", "amazonses.com", "amazon.de", "amazon.fr", "amazon.it", "amazon.es", "amazon.co.jp", "amazon.in", "amazon.com.au", "amazon.com.br", "amazon.com.mx", "amazon.nl", "amazon.pl", "amazon.sg", "amazon.ae", "amazon.sa", "amazon.se", "amazon.eg", "amazon.cn", "amazon.com.be", "amazon.com.tr"], // v6.0.2: all regional domains
    "amazon account": ["amazon.com", "amazon.co.uk", "amazon.ca", "amazonses.com", "amazon.de", "amazon.fr", "amazon.it", "amazon.es", "amazon.co.jp", "amazon.in", "amazon.com.au", "amazon.com.br", "amazon.com.mx", "amazon.nl", "amazon.pl", "amazon.sg", "amazon.ae", "amazon.sa", "amazon.se", "amazon.eg", "amazon.cn", "amazon.com.be", "amazon.com.tr"], // v6.0.2: all regional domains
    "amazon security": ["amazon.com", "amazon.co.uk", "amazon.ca", "amazonses.com", "amazon.de", "amazon.fr", "amazon.it", "amazon.es", "amazon.co.jp", "amazon.in", "amazon.com.au", "amazon.com.br", "amazon.com.mx", "amazon.nl", "amazon.pl", "amazon.sg", "amazon.ae", "amazon.sa", "amazon.se", "amazon.eg", "amazon.cn", "amazon.com.be", "amazon.com.tr"], // v6.0.2: all regional domains
    "netflix support": ["netflix.com"],
    "netflix account": ["netflix.com"],
    "docusign": ["docusign.com", "docusign.net"],
    "adobe sign": ["adobe.com", "adobesign.com"],
    "intuit": ["intuit.com", "tsheets.com", "macpayroll.com"], // v6.0.2: added tsheets.com + macpayroll.com
    "quickbooks": ["intuit.com", "quickbooks.com", "tsheets.com", "macpayroll.com"], // v6.0.2: added tsheets.com + macpayroll.com
    ...
    "lowes": ["lowes.com"],
    "ebay": ["ebay.com", "ebay.co.uk", "ebay.de", "ebay.fr", "ebay.it", "ebay.es", "ebay.com.au", "ebay.ca", "ebay.nl", "ebay.pl", "ebay.ie", "ebay.at", "ebay.ch", "ebay.com.sg"], // v6.0.2: all regional sites
    "dmv": [".gov"],
    ...
    "xbox": ["xbox.com", "microsoft.com", "microsoft365.com"], // v6.0.2: added microsoft365.com
    "xbox live": ["xbox.com", "microsoft.com", "microsoft365.com"], // v6.0.2: added microsoft365.com
    ...
    "disney+": ["disneyplus.com", "disney.com", "d23.com", "disneyonline.com", "disneyaccount.com"],
    "disney plus": ["disneyplus.com", "disney.com", "d23.com", "disneyonline.com", "disneyaccount.com"],
    "hulu": ["hulu.com"],
    ...
    "apple tv": ["apple.com", "icloud.com", "itunes.com"], // v6.0.2: added icloud.com + itunes.com
    "visa": ["visa.com"],
    "visa card": ["visa.com"],
    ...
    "deutsche post": ["deutschepost.de", "dhl.de", "dpdhl.com"], // v6.0.2: added dpdhl.com
    ...
    'disney': ['disney.com', 'disneyplus.com', 'go.com', 'disneyland.disney.go.com', 'disneyworld.disney.go.com', 'd23.com', 'thewaltdisneycompany.com', 'disneyonline.com', 'disneyaccount.com'],
    'walt disney': ['disney.com', 'disneyplus.com', 'go.com', 'disneyland.disney.go.com', 'disneyworld.disney.go.com', 'd23.com', 'thewaltdisneycompany.com', 'disneyonline.com', 'disneyaccount.com'],
    'disneyland': ['disney.com', 'disneyplus.com', 'go.com', 'disneyland.disney.go.com', 'disneyworld.disney.go.com', 'd23.com', 'thewaltdisneycompany.com', 'disneyonline.com', 'disneyaccount.com'],
    'disney world': ['disney.com', 'disneyplus.com', 'go.com', 'disneyland.disney.go.com', 'disneyworld.disney.go.com', 'd23.com', 'thewaltdisneycompany.com', 'disneyonline.com', 'disneyaccount.com'],
...
const CREDENTIAL_REQUEST_PHRASES = [
    ...
    // v6.1.0: Session-expiry language (AiTM lure patterns)
    'session has expired', 'session expired', 'session timeout',
    'your access has been revoked', 'access revoked',
    're-verify your account', 'reverify your account',
    // v6.1.0: QR-based credential phishing language
    'scan to verify', 'scan to authenticate', 'scan to sign in',
    'scan to log in', 'scan to continue', 'scan this code',
    'scan the qr code', 'scan qr code to verify',
    'scan with your phone', 'use your phone to scan'
];
...
// v6.1.0: BEC linguistic boosters. Phrases statistically overrepresented in BEC emails
// and rare in normal American/European business correspondence.
// Viktor: "These are my tells. I can paraphrase payment instructions, but my operators
// keep writing 'kindly process' because that's how they write."
// Booster only — never fires standalone. Feeds Pattern E.
const BEC_LINGUISTIC_PHRASES = [
    'kindly process', 'kindly wire', 'kindly transfer', 'kindly remit',
    'kindly arrange', 'kindly expedite', 'kindly send',
    'do the needful', 'revert back', 'please oblige',
    'treat as urgent', 'at the earliest', 'at your earliest',
    'kindly comply', 'please do the needful',
    'kindly update the bank details', 'kindly effect payment'
];

// v6.1.0: Anti-verification language. Phrases designed to defeat phone callback,
// the single most effective control against wire fraud.
// Viktor: "If I can stop them from picking up the phone, I win every time."
// Booster only — never fires standalone. Feeds Pattern E.
const ANTI_VERIFICATION_PHRASES = [
    'do not call to verify', "don't call to verify",
    'dont call to verify', 'do not call me',
    "don't call me", 'dont call me',
    'email me back instead of calling', 'reply to this email only',
    'do not contact me by phone', 'not available by phone',
    'cannot take calls', "can't take calls",
    'cant take calls', 'unable to take calls',
    "i'm in a meeting", 'im in a meeting',
    "i'm boarding a plane", 'im boarding a plane',
    'handle before i land', "i'll explain later",
    'ill explain later', 'just process it',
    'just handle it', 'in a conference',
    'do not phone', "don't phone"
];

// v6.1.0: Real estate closing pressure. Phrases that manufacture artificial urgency
// around transaction deadlines. Narrowed to unexpected schedule changes — not normal
// deadline language like "closing tomorrow" which is routine in escrow.
// Viktor: "I need the escrow officer to panic and skip verification."
// Booster only — never fires standalone. Feeds Pattern E.
const CLOSING_PRESSURE_PHRASES = [
    'closing moved up', 'closing pushed to today',
    'closing has been accelerated', 'closing has been moved',
    'closing rescheduled to today', 'closing moved to today',
    'need to fund immediately', 'fund immediately or closing will be delayed',
    'wire must arrive today', 'wire must be sent today',
    'title company needs funds today', 'title needs funds immediately',
    'closing in 2 hours', 'closing in one hour',
    'fund before cob today', 'must fund by noon',
    'lender requires funds today', 'funding deadline moved up',
    'escrow needs wire today', 'settlement moved to today'
];
...
const KNOWN_PLATFORM_DOMAINS = [
    ...
    'gotowebinar.com', 'gotomeeting.com',
...
// v6.0.4: Disposable/throwaway email providers.
// These services exist for anonymous, temporary, no-verification email.
// Zero legitimate business use. Zero false positive risk.
// NOT freemail (Gmail, Yahoo, Outlook, Protonmail are real providers with real users).
// Viktor: "I grab a throwaway inbox in 10 seconds. No signup, no trace."
const DISPOSABLE_EMAIL_DOMAINS = [
    // === Guerrilla Mail (all known domains) ===
    ...
    // === Mailinator network (includes published alternative domains) ===
    'mailinator.com', 'mailinator2.com',
    // v6.0.4 Viktor Finding #2: Mailinator operates hundreds of alternative domains.
    // These are all Mailinator-operated, anonymous, auto-expiring inboxes.
    'binkmail.com', 'chammy.info', 'devnullmail.com', 'fiifke.de',
    'grandmamail.com', 'letthemeatspam.com', 'mailtothis.com',
    'notmailinator.com', 'reallymymail.com', 'safetypost.de',
    'thisisnotmyrealemail.com', 'tradermail.info', 'veryreallyfakemail.com',
    'mailinater.com', 'mailinator.net', 'mailinator.org',
    'mailinator.us', 'sogetthis.com', 'reconmail.com',
    'streetwisemail.com', 'bobmail.info', 'binmail.info',
    'garliclife.com', 'mailismagic.com', 'maildx.com',
    'meltmail.com',
    // === Temp Mail / 10 Minute Mail ===
    'tempmail.com', 'temp-mail.org',
    '10minutemail.com', '10minutemail.net',
    // === YOPmail (French-origin, global) ===
    'yopmail.com', 'yopmail.fr', 'yopmail.net',
    // === Other well-known disposable services ===
    'throwaway.email', 'throwaway.me',
    'dispostable.com', 'mailnesia.com', 'maildrop.cc',
    'discard.email', 'fakeinbox.com', 'tempail.com',
    'mohmal.com', 'tempmailo.com', 'emailondeck.com',
    'burnermail.io',
    'trashmail.com', 'trashmail.net', 'trashmail.me',
    'trash-mail.at', 'trash-mail.com', 'mytrashmail.com',
    'mailcatch.com', 'mytemp.email', 'tempr.email',
    'tempinbox.com', 'getairmail.com', 'mailforspam.com',
    'safetymail.info', 'filzmail.com', 'inboxalias.com',
    'jetable.org', 'jetable.com', 'nwldx.com',
    'spamgourmet.com', 'mailexpire.com', 'tempsky.com',
    'harakirimail.com', 'getnada.com', 'emailfake.com',
    'crazymailing.com', 'mintemail.com', 'mailnull.com',
    'spaml.com', 'tempmailaddress.com', 'mailzilla.com',
    'tmail.ws', 'tmpmail.net', 'tmpmail.org',
    'mailsac.com', 'inboxkitten.com', 'tempmailer.com',
    'tempmails.net',
    // === German-market disposable ===
    'wegwerfmail.de', 'wegwerfmail.net', 'einrot.com',
    'hulapla.de', 'fugmail.com',
    // === Known abuse-only providers ===
    'cock.li', 'aaathats3as.com', 'national.shitposting.agency'
];
...
function addUserTrustedDomain(domain) {
    ...
    // v6.0.4: Skip disposable email providers (anonymous throwaway inboxes).
    // Prevents trust list contamination — a disposable domain in the trusted list
    // would suppress org impersonation and contact lookalike on that domain.
    // Viktor: "If guerrillamail.com gets trusted, I bypass two detection paths."
    if (DISPOSABLE_EMAIL_DOMAINS.some(disp => d === disp || d.endsWith('.' + disp))) return;
...
Office.onReady(async (info) => {
    console.log('Email Fraud Detector v5.2.2 (Phase 2 Silent) script loaded, host:', info.host);
...
function detectGibberishDomain(email) {
    ...
    // v6.0.3: Targeted carve-out for short acronym domains with 'y' as quasi-vowel.
    // "mynhd" (5 chars, has y) = legitimate abbreviation → skip.
    // "brtfq" (5 chars, no y, pure consonant gibberish) → still fires +3.
    // "xntrfhd" (7 chars, no vowels) → still fires +3.
    // Viktor: Do NOT count 'y' as a vowel globally (xyrqt, bynkz would clear).
    // Do NOT raise length threshold globally (brtfq would clear). Carve-out is tightest.
    if (mainPart.length > 3) {
        const vowelCount = (mainPart.match(/[aeiou]/gi) || []).length;
        if (vowelCount === 0) {
            const hasY = /y/i.test(mainPart);
            if (!(mainPart.length <= 6 && hasY)) { // v6.0.3: skip only if short + has y
                suspicionScore += 3;
...
function detectBrandImpersonation(subject, body, senderDomain, displayName) {
    ...
    // v6.0.0 Phase 2: Strip legal footers, signatures, and unsubscribe blocks before
    // brand keyword scanning. Brand names in boilerplate are not impersonation signals.
    // Only affects the inBody check. Subject and display name paths are untouched.
    const strippedBodyLower = stripBoilerplateForBrandDetection(bodyLower);
    ...
    for (const [brandName, config] of Object.entries(BRAND_CONTENT_DETECTION)) {
        ...
        const inBody = keywordMatchesText(strippedBodyLower);
        ...
        if (mentionsBrand) {
            ...
            if (!isLegitimate) {
                ...
                // TIER 1a: Brand in display name = sender claims to BE this brand. Flag immediately.
                if (inDisplayName) {
                    console.log('BRAND CHECK TRIGGERED [reason: display-name-brand] -', brandName, 'from', senderDomain);
                    return {
                        brandName: formatEntityName(brandName),
                        senderDomain: senderDomain,
                        legitimateDomains: config.legitimateDomains
                ...
                // TIER 1b: Brand in subject only (display name does NOT contain brand).
                // v5.2.2: Require at least one supporting signal to distinguish real phishing
                // ("Your Macy's account has been suspended!") from legitimate third-party references
                // ("Find your place at the Macy's Thanksgiving Day Parade").
                if (inSubject) {
                    const combinedText = subjectLower + ' ' + bodyLower;
                    const subjectHasUrgency = PHISHING_URGENCY_KEYWORDS.some(phrase =>
                    ...
                    if (subjectHasUrgency || hasSuspiciousDomain) {
                        console.log('BRAND CHECK TRIGGERED [reason: subject-only: ' + (subjectHasUrgency ? 'urgency' : 'suspicious_domain') + '] -', brandName, 'from', senderDomain);
                        return {
                            brandName: formatEntityName(brandName),
                            senderDomain: senderDomain,
                            legitimateDomains: config.legitimateDomains
                    ...
                    console.log('BRAND CHECK SKIPPED [reason: subject-only-no-supporting-signals] -', brandName, 'from', senderDomain);
                    continue;
                }
...
// v6.0.0 Phase 2: Strip legal footers, unsubscribe blocks, and signature blocks
// from body text BEFORE brand keyword scanning. Brand keywords in boilerplate
// (e.g., "wish" in "if you do not wish to continue") are not impersonation signals.
// This function works bottom-up: finds the earliest boilerplate boundary in the
// lower portion of the email and returns everything above it.
// Only used for brand detection — other detection paths use their own stripping.
function stripBoilerplateForBrandDetection(text) {
    if (!text || text.length < 100) return text;
    const lower = text.toLowerCase();
    const len = text.length;
    
    // Only look for boilerplate in the bottom 40% of the email.
    // Legal footers, signatures, and unsubscribe blocks live at the bottom.
    const searchStart = Math.floor(len * 0.6);
    let cutPoint = len;
    
    // === SIGNATURE MARKERS ===
    // Common email signature delimiters. These mark where the author's
    // message ends and their name/title/contact block begins.
    const sigMarkers = [
        /\n--\s*\n/,                           // "-- " (standard sig delimiter)
        /\n-{3,}\s*\n/,                         // "---" or more dashes on own line
    ];
    for (const marker of sigMarkers) {
        const match = lower.substring(searchStart).match(marker);
        if (match) {
            const absPos = searchStart + lower.substring(searchStart).indexOf(match[0]);
            cutPoint = Math.min(cutPoint, absPos);
        }
    }
    
    // Signature phrases: "Best regards,", "Sincerely,", "Thanks," etc.
    // Only match when they appear on their own line (start of line or after newline)
    // to avoid matching "Best regards" in the middle of a sentence.
    const sigPhrases = [
        'best regards', 'kind regards', 'warm regards', 'regards,',
        'sincerely,', 'sincerely yours', 'yours truly', 'yours sincerely',
        'thanks,', 'thank you,', 'many thanks,', 'with thanks,',
        'best wishes,', 'cheers,', 'respectfully,', 'cordially,'
    ];
    for (const phrase of sigPhrases) {
        let idx = lower.indexOf(phrase, searchStart);
        while (idx !== -1) {
            // Verify it's at the start of a line (or near start after whitespace)
            const lineStart = lower.lastIndexOf('\n', idx);
            const textBetween = lower.substring(lineStart + 1, idx).trim();
            if (textBetween.length <= 2) { // Allow up to 2 chars before (e.g., "> ")
                cutPoint = Math.min(cutPoint, lineStart > 0 ? lineStart : idx);
                break;
            }
    ...
    // === LEGAL / UNSUBSCRIBE MARKERS ===
    // These indicate boilerplate legal text, privacy notices, or mailing list footers.
    // Match when found in the bottom portion. Use the earliest match as the cut point.
    const legalMarkers = [
        'unsubscribe',
        ...
        'all rights reserved',
        '© copyright',
        '©2',  // catches ©2024, ©2025, ©2026 etc.
    ];
    for (const marker of legalMarkers) {
        const idx = lower.indexOf(marker, searchStart);
        if (idx !== -1) {
            // Walk back to the start of this line
            const lineStart = lower.lastIndexOf('\n', idx);
            const candidateCut = lineStart > 0 ? lineStart : idx;
            cutPoint = Math.min(cutPoint, candidateCut);
        }
    }
    
    // === CONFIDENTIALITY DISCLAIMERS ===
    // Corporate email footers with legal disclaimers about confidentiality.
    const confPhrases = [
        ...
        'legal disclaimer',
        'important notice:',
        'notice:' // only if followed by legal-ish content
    ];
    for (const phrase of confPhrases) {
        const idx = lower.indexOf(phrase, searchStart);
        if (idx !== -1) {
            // For generic "notice:", verify it's legal context (not "please notice...")
            if (phrase === 'notice:' || phrase === 'important notice:') {
                const nearby = lower.substring(idx, Math.min(idx + 200, len));
                if (!nearby.includes('intended') && !nearby.includes('recipient') &&
                    !nearby.includes('confidential') && !nearby.includes('authorized')) {
                    continue;
                }
            }
            const lineStart = lower.lastIndexOf('\n', idx);
    ...
    // === SAFE FALLBACK ===
    // If stripping would remove more than 80% of the body, something went wrong
    // (tiny email where the "footer" IS the email). Fall back to full body.
    // Phase 1 gate still enforces body-only gating regardless.
    if (cutPoint < len * 0.2) {
        console.log('[BRAND STRIP] Fallback: stripping would remove >' + Math.round((1 - cutPoint/len) * 100) + '% of body, using original');
        return text;
    }
    
    if (cutPoint < len) {
        console.log('[BRAND STRIP] Cut at', cutPoint, '/', len, '(' + Math.round((len - cutPoint)/len * 100) + '% removed)');
    }
...
function _brandBodyHasSupportingSignals(config, bodyLower, subjectLower, senderDomainLower, bodyOriginal) {
    ...
    // v6.0.3: Require liveMentions >= 2 before urgency can fire body-only brand.
    // A single casual mention ("dropped it at FedEx") + unrelated urgency ("within 24 hours"
    // about loan funding) is not brand impersonation. Real phishing repeats the brand.
    // Viktor Finding #1: Threshold 2 (not 3) catches short focused phish with 2 mentions + urgency.
    const combinedText = subjectLower + ' ' + bodyLower;
    const hasUrgency = PHISHING_URGENCY_KEYWORDS.some(phrase => 
        combinedText.includes(phrase.toLowerCase())
    );
    if (hasUrgency && liveMentions >= 2) return 'urgency'; // v6.0.3: was unconditional, now gated
    if (hasUrgency && liveMentions < 2) {
        console.log('[DEBUG v6.0.3] Signal 2 SKIPPED: urgency found but liveMentions=' + liveMentions + ' < 2, brand:', config.keywords[0]);
    }
    
    // Signal 3: Sender domain contains suspicious words (e.g. "secure-paypal-login.com")
    // v6.0.3: Require liveMentions >= 2. Single mention from a suspicious domain is caught
    // by detectSuspiciousDomain independently. Brand impersonation needs brand saturation.
    // Viktor Finding #2: Verified that secure-fedex-verify.com fires via detectSuspiciousDomain
    // (hyphenated part match on "secure"/"verify"). Pre-existing gap for brand-only domains
    // like fedex-tracking.com is logged for future brand-substring detection, not this fix.
    const hasSuspiciousDomain = SUSPICIOUS_DOMAIN_WORDS.some(word => 
        senderDomainLower.includes(word)
    );
    if (hasSuspiciousDomain && liveMentions >= 2) return 'suspicious_domain_words'; // v6.0.3: was unconditional, now gated
    if (hasSuspiciousDomain && liveMentions < 2) {
        console.log('[DEBUG v6.0.3] Signal 3 SKIPPED: suspicious domain but liveMentions=' + liveMentions + ' < 2, brand:', config.keywords[0], 'domain:', senderDomainLower);
    }
...
// v5.x: Returns true if domain appears in any brand's legitimate domains list
// across both BRAND_CONTENT_DETECTION and IMPERSONATION_TARGETS.
// Used to suppress suspicious domain heuristics on first-party brand domains.
function isOfficialBrandDomain(domain) {
    for (const config of Object.values(BRAND_CONTENT_DETECTION)) {
        if (config.legitimateDomains && config.legitimateDomains.some(ld => domain === ld || domain.endsWith('.' + ld))) {
            return true;
        }
    }
    for (const domains of Object.values(IMPERSONATION_TARGETS)) {
        if (domains.some(ld => domain === ld || domain.endsWith('.' + ld))) {
            return true;
        }
    }
    return false;
...
function detectSuspiciousDomain(domain) {
    ...
    // v5.x: Skip suspicious domain heuristics for domains explicitly whitelisted
    // as legitimate brand domains. If disneyaccount.com is in Disney's legit list,
    // the "account" suffix heuristic should not fire on it.
    if (isOfficialBrandDomain(domainLower)) {
        return null;
    }
    ...
    if (suspiciousGenericTLDs.includes(tld)) {
        return {
            pattern: tld,
            reason: `This email was sent from a domain ending in ${tld}. While some legitimate businesses use ${tld}, domains ending in ${tld} have been identified by Spamhaus and Symantec as frequently used in spam and phishing campaigns. If you don't recognize this sender, verify before clicking any links.`
        };
    }
...
// v6.0.1: Person-vs-Org display name classifier (COP-approved rewrite)
// Suppresses Sender Identity Mismatch for individuals. Does NOT suppress
// lookalike/homoglyph, auth, content-risk, or domain-risk detectors.
// Viktor: "Person names buy me nothing. Domain, auth, keyword, and content
//          detectors fire independently. SIM is a supporting signal."
//
// v6.0.1 additions per COP spec:
//   - Internal-cap names: LeAnna, McCaskell, McDonald, DeAndre, MacGregor
//   - Apostrophe names: O'Brien, D'Angelo
//   - Hyphenated names: Jean-Luc, Mary-Kate
//   - Name particles: De La Cruz, van Gogh, von Beethoven
//   - Unicode/diacritics: María, Siobhán, José, François
//   - Total length guard: >50 chars = not a person
//   - Token length guard: per-token 1–30 chars
//   - Arabic/Hebrew/Aramaic particles: bin, bint, ibn, abu, abd, ben, bat, bar
//   - Diacritic normalization on org-indicator check (Viktor: closes "Sécurity Téam" bypass)
//   - Single-token names intentionally allowed (Indonesian mononyms: Suharto, Widodo)
//   - Known limitation: orgIndicators are English-only (future i18n work)
function displayLooksLikePerson(displayName) {
    if (!displayName) return false;
    
    // v6.0.1: Total length guard — real person display names are short
    // v6.0.1-viktor: Raised from 40 to 50 for South Asian and Thai name coverage
    if (displayName.length > 50) return false;
    
    // v6.0.1: Unicode-safe cleaning — preserve accented letters (María, Siobhán, José)
    // Old regex /[^\w\s'-]/g stripped diacritics; \p{L} keeps all Unicode letters
    const cleaned = displayName.replace(/[^\p{L}\s'-]/gu, ' ').trim();
    ...
    // Corporate/org indicators: if ANY word (including after preposition) is one of these, it's an org
    // v6.0.1: Added 'loans' (COP guardrail: prevents "LeAnna — Watermark Home Loans" from passing)
    // Known limitation: orgIndicators are English-only. Org names in Korean, Japanese, Arabic, Hindi
    // pass through this check. Future i18n work: locale-specific org indicator sets for top languages.
    const orgIndicators = new Set([
        ...
        'bank', 'insurance', 'financial', 'credit', 'mortgage', 'lending', 'loans',
    ...
    // v6.0.1-viktor: Normalize diacritics before org lookup to close Unicode bypass.
    // Without this, "Sécurity Téam" bypasses org check because accented chars ≠ "security"/"team".
    // NFD decomposition splits accented chars into base + combining mark, then we strip the marks.
    for (const word of words) {
        const normalized = word.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        if (orgIndicators.has(normalized)) return false;
    }
    ...
    // v6.0.1: Name particles — common middle tokens in multi-part surnames
    // European: María De La Cruz, Vincent van Gogh, Ludwig von Beethoven
    // v6.0.1-viktor: Arabic/Hebrew/Aramaic: Ahmed bin Salman, Omar ibn Khattab, Abu Hassan
    const NAME_PARTICLES = new Set([
        'de', 'di', 'da', 'del', 'dela', 'des', 'du', 'dos', 'das',
        'van', 'von', 'la', 'le', 'el', 'al', 'den', 'der',
        'bin', 'bint', 'ibn', 'abu', 'abd', 'ben', 'bat', 'bar'
    ]);
    
    // v6.0.1: Unified name-token checker — covers all person-name patterns:
    //   Title case:    John, Smith
    //   Internal caps: LeAnna, McCaskell, McDonald, DeAndre, MacGregor
    //   Apostrophe:    O'Brien, D'Angelo
    //   Hyphenated:    Jean-Luc, Mary-Kate
    //   Unicode:       María, Siobhán, José, François
    //   Initials:      J, J.
    //   All caps:      JOHN (2+ chars)
    //   All lower:     john (2+ chars)
    function looksLikeNameToken(w) {
        // v6.0.1-viktor: Raised from 20 to 30 for South Asian/Thai name tokens
        if (w.length < 1 || w.length > 30) return false;
        if (/^\p{Lu}\.?$/u.test(w)) return true; // Initial: "J" or "J."
        if (w.length < 2) return false;
        if (/^\p{Lu}+$/u.test(w)) return true; // All caps: "JOHN"
        if (/^\p{Ll}[\p{L}'-]*$/u.test(w)) return true; // All lower or lower-start with diacritics: "josé"
        // General name pattern: starts uppercase, has lowercase, letters/apostrophes/hyphens only
        // Catches: LeAnna, McCaskell, O'Brien, Jean-Luc, María, Siobhán
        if (/^\p{Lu}/u.test(w) && /\p{Ll}/u.test(w) && /^[\p{L}'-]+$/u.test(w)) return true;
        return false;
    }
    
    // v5.2.0 SIM-06 + v6.0.1: Check name words. Each must be a valid name token
    // or a name particle (non-boundary positions only for 4-token names).
    // At least one word must be a real name token (can't be all particles).
    // v6.0.1-viktor: Single-token names (nameWords.length === 1) are intentionally allowed.
    // Indonesian/Javanese mononyms (Suharto, Widodo, Sukarno) are normal display names.
    // Do NOT add a >= 2 requirement — it would break mononym support for ~270M Indonesians.
    if (nameWords.length >= 1 && nameWords.length <= 3) {
        // 1–3 tokens: each must be a name token or a particle
        const allValid = nameWords.every(w => looksLikeNameToken(w) || NAME_PARTICLES.has(w.toLowerCase()));
        const hasRealName = nameWords.some(w => looksLikeNameToken(w));
        if (allValid && hasRealName) return true;
    }
    
    // v6.0.1: 4-token names — require first and last to be name tokens,
    // middle tokens can be particles or name tokens. At least one middle must be a particle.
    // This prevents 4 random title-case words from passing (suspicious), but allows
    // "María De La Cruz" and "Jean Pierre Van Damme".
    if (nameWords.length === 4) {
        if (looksLikeNameToken(nameWords[0]) && looksLikeNameToken(nameWords[3])) {
            const mid1Valid = looksLikeNameToken(nameWords[1]) || NAME_PARTICLES.has(nameWords[1].toLowerCase());
            const mid2Valid = looksLikeNameToken(nameWords[2]) || NAME_PARTICLES.has(nameWords[2].toLowerCase());
            const hasParticle = NAME_PARTICLES.has(nameWords[1].toLowerCase()) || NAME_PARTICLES.has(nameWords[2].toLowerCase());
            if (mid1Valid && mid2Valid && hasParticle) return true;
        }
    }
...
function detectDisplayNameDomainMismatch(displayName, senderDomain) {
    ...
    if (displayLooksLikePerson(displayName)) {
        // v6.0.1: COP-required logging for person-name suppression
        console.log('SENDER IDENTITY SKIPPED [reason: personal-name-detected] - "' + displayName + '" from ' + senderDomain);
        return null;
    }
...
// v6.0.4: Disposable email provider detection.
// These services exist for anonymous, temporary, no-verification email.
// Zero legitimate business use. Zero false positive risk.
// Viktor: "I grab a throwaway inbox in 10 seconds. No signup, no trace."
function detectDisposableEmailSender(senderDomain) {
    if (!senderDomain) return null;
    const domainLower = senderDomain.toLowerCase();
    for (const d of DISPOSABLE_EMAIL_DOMAINS) {
        if (domainLower === d || domainLower.endsWith('.' + d)) {
            console.log('DISPOSABLE EMAIL DETECTED -', domainLower, '(matched:', d, ')');
            return { senderDomain: domainLower, provider: d };
        }
    }
    return null;
}
...
function detectContactLookalike(senderEmail) {
    ...
    // v6.0.3: Freemail carve-out. Trusted domain skip is correct for private domains
    // (you send to @company.com, so @company.com is trusted, lookalike check unnecessary).
    // But freemail domains (gmail, yahoo, etc.) are trusted from Sent Items AND anyone can
    // register an account. journey.brennan@gmail.com impersonating journeybrennan@gmail.com
    // is the #1 wire fraud vector in escrow. Never skip freemail lookalike detection.
    // Viktor: "This is the attack that costs your industry real money."
    if (isTrustedDomain(senderDomain) && !publicDomains.includes(senderDomain)) return null;
    ...
    for (const contact of knownContacts) {
        ...
        // v6.0.3: FREEMAIL CARVE-OUT. On public email domains, the username IS the
        // identity. Two similar usernames on gmail.com are not "coworkers" — they're
        // potentially an attacker impersonating your client. The skip only applies to
        // non-public domains where multiple employees legitimately share a domain.
        if (senderDomain === contactDomain && !publicDomains.includes(senderDomain)) {
            continue;
        }
        
        // v6.0.3: Same-domain freemail username lookalike. On public email domains,
        // compare usernames directly. "journeybrennan" vs "journey.brennan" on gmail.com
        // = 1 edit apart. That's impersonation, not coincidence.
        // Matches content.js which already had this check (was reachable there).
        if (senderDomain === contactDomain && publicDomains.includes(senderDomain)
            && usernameDiff > 0 && usernameDiff <= 4) {
            return {
...
// v6.1.0: BEC linguistic booster detection.
// Booster only — feeds Pattern E, never fires standalone.
function detectPhase2BECLanguage(content) {
    if (!content) return null;
    const lowerContent = content.toLowerCase();
    const collapsedContent = collapseForMatch(lowerContent);
    const matched = [];
    for (const phrase of BEC_LINGUISTIC_PHRASES) {
        if (phraseMatchesContent(lowerContent, collapsedContent, phrase)) matched.push(phrase);
    }
    if (matched.length === 0) return null;
    console.log('BEC LINGUISTIC BOOSTER -', matched.length, 'matches:', matched.join(', '));
    return { signal: 'bec_linguistic', matched: matched };
}

// v6.1.0: Anti-verification language detection.
// These phrases defeat phone callback — the #1 control against wire fraud.
// Booster only — feeds Pattern E, never fires standalone.
function detectPhase2AntiVerificationLanguage(content) {
    if (!content) return null;
    const lowerContent = content.toLowerCase();
    const collapsedContent = collapseForMatch(lowerContent);
    const matched = [];
    for (const phrase of ANTI_VERIFICATION_PHRASES) {
        if (phraseMatchesContent(lowerContent, collapsedContent, phrase)) matched.push(phrase);
    }
    if (matched.length === 0) return null;
    console.log('ANTI-VERIFICATION BOOSTER -', matched.length, 'matches:', matched.join(', '));
    return { signal: 'anti_verification', matched: matched };
}

// v6.1.0: Real estate closing pressure detection.
// Phrases that manufacture artificial urgency around transaction deadlines.
// Narrowed to unexpected schedule changes, not routine deadline language.
// Booster only — feeds Pattern E, never fires standalone.
function detectPhase2ClosingPressure(content) {
    if (!content) return null;
    const lowerContent = content.toLowerCase();
    const collapsedContent = collapseForMatch(lowerContent);
    const matched = [];
    for (const phrase of CLOSING_PRESSURE_PHRASES) {
        if (phraseMatchesContent(lowerContent, collapsedContent, phrase)) matched.push(phrase);
    }
    if (matched.length === 0) return null;
    console.log('CLOSING PRESSURE BOOSTER -', matched.length, 'matches:', matched.join(', '));
    return { signal: 'closing_pressure', matched: matched };
}

// v6.1.0: Document-shared lure detection.
// AiTM credential theft entry vector — email is ABOUT sharing a document
// but sender domain doesn't match the platform link destination.
// Booster for Patterns A and B.
function detectPhase2DocumentLure(content) {
    if (!content) return null;
    const lowerContent = content.toLowerCase();
    const collapsedContent = collapseForMatch(lowerContent);
    const DOCUMENT_LURE_PHRASES = [
        'shared a document with you', 'shared a file with you',
        'shared document with you', 'shared file with you',
        'review this document', 'view shared document',
        'access your document', 'view the document',
        'open shared file', 'document is ready for review',
        'file has been shared', 'document has been shared',
        'click to view document', 'click to access document',
        'view in onedrive', 'view in sharepoint',
        'view in google drive', 'open in dropbox'
    ];
    const matched = [];
    for (const phrase of DOCUMENT_LURE_PHRASES) {
        if (phraseMatchesContent(lowerContent, collapsedContent, phrase)) matched.push(phrase);
    }
    if (matched.length === 0) return null;
    console.log('DOCUMENT LURE BOOSTER -', matched.length, 'matches:', matched.join(', '));
    return { signal: 'document_lure', matched: matched };
}
...
function evaluatePatternA(signals) {
    const { credentialLanguage, senderLinkMismatch, urls, documentLure } = signals; // v6.1.0: documentLure
    if (!credentialLanguage) return null;
    if (!urls || urls.length === 0) return null;
    if (!senderLinkMismatch) return null;
    // v6.1.0: Document lure boosts confidence from high to critical.
    // "Shared a document with you" + credential language + domain mismatch = AiTM attack.
    const confidence = documentLure ? 'critical' : 'high';
    return {
        patternId: 'pattern_a_credential_harvesting',
        patternName: 'Credential Harvesting Attempt',
        confidence: confidence,
        signals: { credential_language: credentialLanguage, sender_link_mismatch: senderLinkMismatch, links_present: { count: urls.length }, document_lure: documentLure || null },
        description: documentLure
            ? 'This email claims to share a document but asks for login credentials through links pointing to domains that don\'t match the sender. This is a common technique to steal your password.'
            : 'This email asks for login credentials and contains links pointing to domains that don\'t match the sender.',
...
function evaluatePatternB(signals, coreWarnings, preGateBrandResult) {
    const { credentialLanguage, suspiciousHostingLink, legitPlatformLink, documentLure } = signals; // v6.1.0
    const hasBrandInWarnings = coreWarnings.some(w => w.type === 'brand-impersonation');
    // v6.0.0: Phase 1 may gate body-only brand matches out of coreWarnings.
    const hasPreGateBrand = preGateBrandResult != null;
    const hasBrandMismatch = hasBrandInWarnings || hasPreGateBrand;
    if (!hasBrandMismatch) return null;
    const brandSource = hasBrandInWarnings ? { fromCore: true } : { fromPreGate: true, gatedReason: 'body-only-no-gates' };
    if (suspiciousHostingLink) {
        const hasUrgency = coreWarnings.some(w => w.type === 'phishing-urgency');
        if (credentialLanguage || hasUrgency || documentLure) { // v6.1.0: documentLure qualifies
            return {
                patternId: 'pattern_b_brand_free_hosting',
                patternName: 'Brand Impersonation with Suspicious Link',
                confidence: documentLure ? 'critical' : 'high', // v6.1.0: doc lure = critical
                signals: { brand_mismatch: brandSource, suspicious_hosting_link: suspiciousHostingLink, credential_language: credentialLanguage || null, urgency: hasUrgency || null, document_lure: documentLure || null },
                description: documentLure
                    ? 'This email impersonates a known brand and claims to share a document, but links to a free hosting platform. This is a common credential theft technique.'
                    : 'This email impersonates a known brand but links to a free hosting platform.',
    ...
    if (legitPlatformLink) {
        ...
        if (documentLure) supportingSignalCount++; // v6.1.0
        if (supportingSignalCount >= 2) {
            return {
                patternId: 'pattern_b_brand_free_hosting',
                patternName: 'Brand Impersonation with Suspicious Link',
                confidence: 'medium',
                signals: { brand_mismatch: brandSource, legit_platform_link: legitPlatformLink, supporting_signal_count: supportingSignalCount, document_lure: documentLure || null },
...
function evaluatePatternC(signals, coreWarnings, preGateBrandResult) {
    ...
    const hasBrandMismatch = coreWarnings.some(w => w.type === 'brand-impersonation') || preGateBrandResult != null;
...
function evaluatePatternD(signals, coreWarnings, preGateBrandResult) {
    ...
    if (coreWarnings.some(w => w.type === 'brand-impersonation') || preGateBrandResult != null) supportingSignals.push('brand_mismatch');
...
function evaluatePatternE(signals, coreWarnings) {
    const { paymentChangeLanguage, bankingTokensOnly, secrecyLanguage,
            becLanguage, antiVerification, closingPressure } = signals; // v6.1.0: new boosters
    ...
    // v6.1.0: Expanded identity signal gate. Originally only reply-to mismatch and
    // on-behalf-of. Now includes all identity-layer signals that indicate the sender
    // isn't who they claim to be. The three-leg requirement (identity + payment + urgency/secrecy)
    // stays intact — we're just widening which identity signals qualify.
    // Viktor: "Lookalike domain is arguably a stronger signal than reply-to mismatch."
    ...
    const hasLookalikeDomain = coreWarnings.some(w => w.type === 'lookalike-domain');
    const hasContactLookalike = coreWarnings.some(w => w.type === 'contact-lookalike');
    const hasImpersonation = coreWarnings.some(w => w.type === 'impersonation');
    const hasAuthFailure = coreWarnings.some(w => w.type === 'auth-failure');
    const hasNewDomain = coreWarnings.some(w => w.type === 'new-domain');
    
    const hasIdentitySignal = hasReplyToMismatch || hasOnBehalfOf || hasLookalikeDomain ||
        hasContactLookalike || hasImpersonation || hasAuthFailure || hasNewDomain;
    if (!hasIdentitySignal) return null;
    
    // v6.1.0: Expanded pressure/secrecy gate. Original: urgency OR secrecy.
    // Now: urgency OR secrecy OR anti-verification OR closing pressure OR BEC linguistic.
    // Anti-verification ("don't call to verify") is the most diagnostic phrase in wire fraud.
    // Closing pressure ("closing moved up") manufactures artificial deadline panic.
    // BEC linguistic ("kindly process") is a statistical BEC marker.
    const hasUrgency = coreWarnings.some(w => w.type === 'phishing-urgency');
    const hasPressureOrSecrecy = hasUrgency || secrecyLanguage || antiVerification ||
        closingPressure || becLanguage;
    if (!hasPressureOrSecrecy) return null;
    
    // Confidence scoring: primary path (explicit payment phrases) = critical.
    // Alternate path (banking tokens only) = high.
    // Anti-verification language bumps alternate path to critical — it's that diagnostic.
    let confidence = paymentChangeLanguage ? 'critical' : 'high';
    if (antiVerification && confidence === 'high') confidence = 'critical'; // v6.1.0
    
    // v6.1.0: Identify which identity signal fired for reporting
    const identitySignals = [];
    if (hasReplyToMismatch) identitySignals.push('reply_to_mismatch');
    if (hasOnBehalfOf) identitySignals.push('on_behalf_of');
    if (hasLookalikeDomain) identitySignals.push('lookalike_domain');
    if (hasContactLookalike) identitySignals.push('contact_lookalike');
    if (hasImpersonation) identitySignals.push('impersonation');
    if (hasAuthFailure) identitySignals.push('auth_failure');
    if (hasNewDomain) identitySignals.push('new_domain');
    ...
    return {
        ...
        confidence: confidence,
        signals: {
            payment_change_language: paymentChangeLanguage || null,
            banking_tokens: bankingTokensOnly || null,
            identity_signals: identitySignals, // v6.1.0: expanded
            urgency: hasUrgency || null,
            secrecy_language: secrecyLanguage || null,
            bec_linguistic: becLanguage || null, // v6.1.0
            anti_verification: antiVerification || null, // v6.1.0
            closing_pressure: closingPressure || null // v6.1.0
        },
        description: antiVerification
            ? 'This email requests payment changes, uses a spoofed identity, AND explicitly discourages phone verification. This is a textbook BEC attack designed to prevent you from confirming the instructions.'
            : paymentChangeLanguage 
...
function runPhase2Engine(emailData, coreWarnings, preGateBrandResult) {
    ...
    const urls = [...extractPhase2Urls(body), ...(emailData.qrUrls || [])]; // v6.2.0: Include QR-decoded URLs
    ...
    const signals = {
        ...
        becLanguage: detectPhase2BECLanguage(body), // v6.1.0
        antiVerification: detectPhase2AntiVerificationLanguage(body), // v6.1.0
        closingPressure: detectPhase2ClosingPressure(body), // v6.1.0
        documentLure: detectPhase2DocumentLure(body), // v6.1.0
        urls: urls,
    ...
    const pB = evaluatePatternB(signals, coreWarnings, preGateBrandResult); if (pB) matchedPatterns.push(pB);
    const pC = evaluatePatternC(signals, coreWarnings, preGateBrandResult); if (pC) matchedPatterns.push(pC);
    const pD = evaluatePatternD(signals, coreWarnings, preGateBrandResult); if (pD) matchedPatterns.push(pD);
...
async function analyzeCurrentEmail() {
    ...
    try {
        ...
        // v6.2.0: Fetch HTML body (for QR decode) and text body in PARALLEL.
        // Both fire simultaneously; processing starts when both complete.
        // Viktor-test: HTML fetch failure doesn't block analysis — QR decode is optional.
        let htmlBody = null;
        let textBody = '';
        let fetchCount = 0;
        const FETCHES_NEEDED = 2;

        function onBodyFetchComplete() {
            fetchCount++;
            if (fetchCount < FETCHES_NEEDED) return;
            // Both fetches done — proceed with analysis
            if (item.getAllInternetHeadersAsync) {
                item.getAllInternetHeadersAsync((headerResult) => {
                    ...
                    const emailData = {
                        ...
                        subject: subject,
                        body: textBody,
                        htmlBody: htmlBody,  // v6.2.0: For QR code detection
                        replyTo: replyTo,
        ...
        item.body.getAsync(Office.CoercionType.Html, (htmlResult) => {
            htmlBody = (htmlResult.status === Office.AsyncResultStatus.Succeeded) ? htmlResult.value : null;
            onBodyFetchComplete();
        });
        item.body.getAsync(Office.CoercionType.Text, (bodyResult) => {
            textBody = (bodyResult.status === Office.AsyncResultStatus.Succeeded) ? bodyResult.value || '' : '';
            onBodyFetchComplete();
        });
...
// v6.1.0: QUOTED REPLY STRIPPING
// Strips quoted reply chains so EFA only scans new message content.
// A six-deep reply thread was scanning the same keywords six times — all on content
// the user already saw when each prior email arrived. This caused false positives
// (flagging keywords from earlier messages the current sender didn't write) and
// 3-second scan times on long threads.
// Viktor approved: attackers can't hide payload in fake quotes because quoted content
// renders visually as old/collapsed text — users don't act on it.
function extractNewContent(bodyHtml) {
    if (!bodyHtml) return bodyHtml;
    
    // HTML boundary markers (order matters — check most specific first)
    const htmlBoundaries = [
        // Gmail quote wrapper
        /<div\s+class\s*=\s*["']gmail_quote["']/i,
        // Outlook Web App reply/forward markers
        /<div\s+id\s*=\s*["']divRplyFwdMsg["']/i,
        /<div\s+id\s*=\s*["']appendonsend["']/i,
        // Outlook desktop border separator (solid top border = reply separator)
        /<div\s+style\s*=\s*["'][^"']*border-top\s*:\s*solid\s+#[eE]1[eE]1[eE]1/i,
        // Generic blockquote (common in many clients)
        /<blockquote[\s>]/i,
        // Outlook classic
        /<!--\s*Original Message\s*-->/i,
    ];
    
    for (const boundary of htmlBoundaries) {
        const match = bodyHtml.match(boundary);
        if (match && match.index !== undefined) {
            const stripped = bodyHtml.substring(0, match.index);
            // Only strip if we'd keep a meaningful amount of content (at least 20 chars of text)
            const textContent = stripped.replace(/<[^>]+>/g, '').trim();
            if (textContent.length >= 20) {
                console.log('QUOTE STRIP (HTML) - kept', textContent.length, 'chars, stripped at', match[0].substring(0, 40));
                return stripped;
            }
        }
    }
    
    // Plain text boundary markers (for plain text emails or after HTML stripping)
    const textBoundaries = [
        // "On [date], [name] wrote:" — Gmail plain text
        /\n\s*On\s+.{10,80}\s+wrote:\s*\n/i,
        // Outlook plain text
        /\n\s*-{3,}\s*Original Message\s*-{3,}/i,
        // "From: ... Sent: ... To: ..." block (Outlook forward headers)
        /\n\s*From:\s+.{5,60}\n\s*Sent:\s+/i,
    ];
    ...
    for (const boundary of textBoundaries) {
        ...
        if (match && match.index !== undefined) {
            ...
            const textContent = stripped.replace(/<[^>]+>/g, '').trim();
            if (textContent.length >= 20) {
                console.log('QUOTE STRIP (text) - kept', textContent.length, 'chars');
                return stripped;
            }
        }
    }
    
    // No quote boundary found — return full body
    return bodyHtml;
}
...
async function processEmail(emailData) {
    ...
    // v6.1.0: Strip quoted reply chains first. Only scan new message content.
    const rawBody = emailData.body || '';
    const newContentBody = extractNewContent(rawBody);
    const { cleanText, urlHosts } = prepareBodyForScanning(newContentBody);
    ...
    if (reputationResult) {
        ...
        if (reputationResult.layer === 'whois_age') {
            ...
            if (ageDays < 7) {
                ...
                severity = 'critical'; // v6.1.0: Client-side severity override
            } else if (ageDays < 30) {
                ...
                severity = 'high'; // v6.1.0: Was falling through header — now counted
            } else {
    ...
    if (ENABLE_PHISHING_WORDS) {
        ...
        if (phishingUrgency) {
            ...
            const hasExtLink = hasExternalLinkMismatch(newContentBody || cleanText, senderDomain);
    ...
    // v6.0.4: Disposable email provider detection
    const disposableEmail = senderDomain ? detectDisposableEmailSender(senderDomain) : null;
    if (disposableEmail) {
        warnings.push({
            type: 'disposable-email',
            severity: 'critical',
            title: 'Disposable Email Address',
            description: `This email was sent from ${disposableEmail.provider}, a disposable email service that requires no identity verification. These temporary inboxes are used to send untraceable messages. No legitimate business correspondent uses throwaway email.`,
            senderEmail: senderEmail,
            provider: disposableEmail.provider
        });
    }
    ...
    if (!providerFlagged && newContentBody) {
        const bodyLower = newContentBody.toLowerCase();
    ...
    // v6.0.0: Moved suspicious domain detection before brand detection
    // so it's available as a gate for body-only brand matches (Phase 1).
    ...
    let preGateBrandResult = null; // v6.0.0: Preserved for Phase 2 Pattern B/C/D
    if (!isTrustedDomain(senderDomain)) {
        ...
        if (brandImpersonation) {
            // v6.0.0: Save pre-gate result so Phase 2 patterns can see body-only brand
            // matches that were gated out. Pattern B specifically needs this to catch
            // "body brand + suspicious hosting link" combinations where the brand was
            // body-only and no other core warning existed to open the gate.
            preGateBrandResult = brandImpersonation;
            
            // v6.0.0 Phase 1: Body-only brand matches MUST NOT fire without ≥1 external suspicion gate.
            // Body-only = brand found in body text but NOT in display name or subject.
            // Gate = another detection system has already flagged something suspicious about this email.
            // This eliminates false positives from casual brand mentions (GoDaddy "wish", Macy's parade, eBay escrow).
            // Display name (Tier 1a) and subject-line (Tier 1b) paths are unaffected.
            if (brandImpersonation.bodyOnly) {
                const BODY_BRAND_GATE_WARNING_TYPES = [
                    'phishing-urgency',
                    'replyto-mismatch',
                    'auth-failure',
                    'gibberish-domain',
                    'gibberish-username',
                    'free-hosting-sender',
                    'disposable-email', // v6.0.4
                    'fake-tld',
                    'suspicious-domain',
                    'via-routing',
                    'provider-flagged'
                ];
                const firedGates = warnings.filter(w => BODY_BRAND_GATE_WARNING_TYPES.includes(w.type)).map(w => w.type);
                if (firedGates.length === 0) {
                    console.log('BRAND CHECK SKIPPED [reason: body-only-no-gates] -', brandImpersonation.brandName, 'from', senderDomain);
                    brandImpersonation = null;
                } else {
                    console.log('BRAND CHECK ELEVATED [reason: body-booster+gates:', firedGates.join(','), '] -', brandImpersonation.brandName, 'from', senderDomain);
                }
            }
    ...
    // v6.1.1: BUGFIX — strippedContent hoisted out of ENABLE_DANGEROUS_KEYWORDS block
    // so payment-change-alert (which runs unconditionally) can also use it.
    ...
    // v6.1.0: STANDALONE PAYMENT CHANGE DETECTION (Anti-VEC control).
    // Pattern E requires identity signals, but VEC attacks come from real compromised
    // mailboxes — no identity signal fires. This standalone path fires whenever
    // PAYMENT_CHANGE_PHRASES appear in the email body, REGARDLESS of sender trust.
    // Viktor: "My best play is a real mailbox with real wire instructions. If you
    // only fire when you see impersonation, I walk right through with the real thing."
    // This is NOT a keyword-only warning (not in keywordTypes), so the keyword-only
    // suppression filter cannot wipe it. Intentionally unfilterable.
    {
        const bodyForPaymentScan = (strippedContent || newContentBody || '').toLowerCase();
        const bodyCollapsed = collapseForMatch(bodyForPaymentScan);
        const matchedPaymentPhrases = [];
        for (const phrase of PAYMENT_CHANGE_PHRASES) {
            if (phraseMatchesContent(bodyForPaymentScan, bodyCollapsed, phrase)) {
                matchedPaymentPhrases.push(phrase);
            }
        }
        if (matchedPaymentPhrases.length > 0) {
            console.log('PAYMENT CHANGE ALERT -', matchedPaymentPhrases.length, 'matches:', matchedPaymentPhrases.join(', '));
            warnings.push({
                type: 'payment-change-alert',
                severity: 'critical',
                title: 'Payment Instructions Changed',
                description: 'This email requests a change to payment or wire instructions. This is the #1 method used in wire fraud — even when the sender appears legitimate. ALWAYS verify payment changes by phone using a number you already have on file.',
                senderEmail: senderEmail,
                matchedPhrases: matchedPaymentPhrases,
                keywords: matchedPaymentPhrases.slice(0, 5),
                keywordExplanation: 'Payment instruction changes are the core mechanism of wire fraud. Attackers compromise real email accounts and send updated bank details from trusted addresses. The only defense is phone verification using a known number.'
            });
        }
    }
    ...
    // ============================================
    // v6.2.0: QR CODE PHISHING ANALYSIS
    // Decode QR codes from email images, analyze embedded URLs.
    // Runs AFTER brand detection (needs brand result for mismatch check)
    // and BEFORE Phase 2 (so decoded URLs feed into pattern engine).
    // Viktor-test: No trust bypass. Compromised mailbox + QR = VEC front door.
    // ============================================
    let qrDecodedUrls = [];
    if (emailData.htmlBody) {
        try {
            qrDecodedUrls = await decodeQRCodesFromHtml(emailData.htmlBody);
            if (qrDecodedUrls.length > 0) {
                console.log('[EFA QR v6.2.0] Decoded', qrDecodedUrls.length, 'URLs from QR codes');
                const qrWarnings = await analyzeQRUrls(qrDecodedUrls, senderDomain, senderEmail, preGateBrandResult || brandImpersonation);
                for (const qw of qrWarnings) {
                    warnings.push(qw);
                }
            }
        } catch (e) {
            console.log('[EFA QR v6.2.0] QR analysis error:', e.message);
        }
    }
    ...
    const phase2EmailData = {
        body: newContentBody, // v6.1.0: Stripped quoted replies — scan new content only
        senderDomain: senderDomain,
        attachments: emailData.attachments || [],
        isKnownContact: isKnownContact,
        platform: 'outlook',
        qrUrls: qrDecodedUrls  // v6.2.0: Feed QR-decoded URLs to Phase 2
    };
    
    const phase2Result = runPhase2Engine(phase2EmailData, warnings, preGateBrandResult);
    ...
    if (nonKeywordWarnings.length === 0 && finalWarnings.length > 0) {
        ...
        // v6.1.0 BUG FIX: Removed isTrustedDomain(senderDomain) from this check.
        // A compromised vendor mailbox IS trusted (learned from Sent Items), and wire-fraud
        // keywords from that mailbox were being silently wiped. This is the VEC kill shot
        // Viktor identified: real domain, real thread, changed bank details, zero warnings.
        // Legit brand domains and known platforms are safe to suppress (they own the brand).
        // Trusted domains are NOT safe to suppress (they can be compromised).
    ...
    // v6.2.0: Also include qr-phishing in the active warning set for Phase 2 gate checks.
...
function displayResults(warnings) {
    ...
    const highCount = warnings.filter(w => w.severity === 'high').length; // v6.1.0: high severity was falling through
    ...
    if (criticalCount > 0 || highCount > 0 || mediumCount > 0) {
        const totalWarnings = criticalCount + highCount + mediumCount;
    ...
    if (warnings.length > 0) {
        ...
        const WARNING_PRIORITY = {
            ...
            'recipient-spoof': 4,
            'homoglyph': 5,
            'qr-phishing': 5,  // v6.2.0: QR phishing is Tier 1/2 — decoded malicious URL from image
            ...
            'lookalike-domain': 10,
            'new-domain': 10, // v6.1.0: Moved from Tier 4 (19) to Tier 2. Domain age is unfakeable.
            // Tier 3 — Strong but context-dependent (boosters, not solo headliners)
            'wire-fraud': 11,
            'payment-change-alert': 11, // v6.1.0: Same tier as wire-fraud, anti-VEC
        ...
        const TIER_1_2_TYPES = new Set(['known-threat', 'phase2-phishing-pattern', 'provider-flagged',
            'recipient-spoof', 'homoglyph', 'qr-phishing', 'replyto-mismatch', 'contact-lookalike',
            'impersonation', 'recipient-domain-impersonation', 'lookalike-domain']);
        // v6.1.0: Force-include payment-change-alert alongside wire-fraud
        const MONEY_TYPES = ['wire-fraud', 'payment-change-alert'];
        for (const moneyType of MONEY_TYPES) {
            const moneyIdx = warnings.findIndex(w => w.type === moneyType);
            if (moneyIdx >= 4 && warnings.some(w => TIER_1_2_TYPES.has(w.type))) {
                if (warnings.length > 3 && !TIER_1_2_TYPES.has(warnings[3].type)) {
                    const [mw] = warnings.splice(moneyIdx, 1);
                    warnings.splice(3, 0, mw);
                    break; // Only force one money warning into slot 4
                }
            }
        }
        ...
        warningsList.innerHTML = warnings.slice(0, 4).map(w => {
            ...
            if (w.type === 'phase2-phishing-pattern' && w.details) {
                ...
                // v6.1.0: Post-compromise guidance for Pattern E (BEC/wire fraud).
                // Collapsible panel with actionable recovery steps. The 71% FBI RAT
                // freeze rate makes this genuinely high-value if the warning came too late.
                const isPatternE = w.details.patterns.some(p => p.patternId === 'pattern_e_payment_redirect');
                const recoveryHtml = isPatternE ? `
                    <details class="warning-recovery" style="margin-top:8px;border-top:1px solid rgba(0,0,0,0.1);padding-top:8px;">
                        <summary style="cursor:pointer;font-weight:bold;font-size:12px;color:#b91c1c;">⚠ If You Already Sent Funds</summary>
                        <div style="font-size:11px;margin-top:6px;line-height:1.5;">
                            <strong>1. Contact your bank immediately.</strong> Request a SWIFT recall or wire reversal. Time is critical — most banks can freeze wires within 24-72 hours.<br>
                            <strong>2. File at ic3.gov.</strong> Submit a complaint and request the FBI Recovery Asset Team (RAT). They have a 71% success rate freezing fraudulent wires.<br>
                            <strong>3. Verify independently.</strong> Call your escrow officer, attorney, or counterparty at a phone number you already have on file. Never use contact info from the suspicious email.
                        </div>
                    </details>
                ` : '';
                emailHtml = `
                ...
                // v6.1.0: Show recovery guidance when wire-fraud appears alongside identity signals.
                const IDENTITY_TYPES = ['replyto-mismatch', 'on-behalf-of', 'lookalike-domain',
                    'contact-lookalike', 'impersonation', 'auth-failure', 'homoglyph'];
                const hasIdentityWarning = w.type === 'wire-fraud' && warnings.some(ow => IDENTITY_TYPES.includes(ow.type));
                const wireRecoveryHtml = hasIdentityWarning ? `
                    <details class="warning-recovery" style="margin-top:8px;border-top:1px solid rgba(0,0,0,0.1);padding-top:8px;">
                        <summary style="cursor:pointer;font-weight:bold;font-size:12px;color:#b91c1c;">⚠ If You Already Sent Funds</summary>
                        <div style="font-size:11px;margin-top:6px;line-height:1.5;">
                            <strong>1. Contact your bank immediately.</strong> Request a SWIFT recall or wire reversal.<br>
                            <strong>2. File at ic3.gov.</strong> Request the FBI Recovery Asset Team (RAT).<br>
                            <strong>3. Verify independently.</strong> Call your counterparty at a number you already have on file.
                        </div>
                    </details>
                ` : '';
                emailHtml = `
                    ...
                    ${wireRecoveryHtml}
                `;
            } else if (w.type === 'payment-change-alert' && w.keywords) {
                // v6.1.0: Standalone payment change warning — always shows recovery panel.
                const keywordTags = w.keywords.slice(0, 5).map(k => 
                    `<span class="keyword-tag">${escapeHtml(k)}</span>`
                ).join('');
                emailHtml = `
                    <div class="warning-keywords-section">
                        <div class="warning-keywords-label">Payment change language:</div>
                        <div class="warning-keywords">${keywordTags}</div>
                    </div>
                    <div class="warning-advice">
                        <strong>STOP.</strong> ${escapeHtml(w.keywordExplanation)}
                    </div>
                    <details class="warning-recovery" style="margin-top:8px;border-top:1px solid rgba(0,0,0,0.1);padding-top:8px;">
                        <summary style="cursor:pointer;font-weight:bold;font-size:12px;color:#b91c1c;">⚠ If You Already Sent Funds</summary>
                        <div style="font-size:11px;margin-top:6px;line-height:1.5;">
                ...
                `;
            } else if (w.type === 'qr-phishing') {
                emailHtml = `
                    <div class="warning-emails">
                        <div class="warning-email-row">
                            <span class="warning-email-label">QR code links to:</span>
                            <span class="warning-email-value suspicious">${escapeHtml(w.qrDomain)}</span>
                        </div>
                        <div class="warning-email-row">
                            <span class="warning-email-label">Full URL:</span>
                            <span class="warning-email-value suspicious">${escapeHtml(truncateText(w.qrUrl, 60))}</span>
                        </div>
                        ${w.qrFlags && w.qrFlags.length > 0 ? `
                        <div class="warning-email-row">
                            <span class="warning-email-label" style="font-size: 11px; margin-top: 4px;">Reasons: ${escapeHtml(w.qrFlags.join(', '))}</span>
                        </div>
                        ` : ''}
                        <div class="warning-email-row">
                            <span class="warning-email-label" style="font-size: 11px; margin-top: 4px;">Do not scan QR codes from unexpected emails. This code links to a potentially dangerous website.</span>
                        </div>
                    </div>
                `;
                ...
                `;
            } else if (w.type === 'provider-flagged' || w.type === 'gibberish-username' || w.type === 'disposable-email') {
                emailHtml = `
                    <div class="warning-emails">
                        <div class="warning-email-row">
                            <span class="warning-email-label">Sender:</span>
