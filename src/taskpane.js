...
Office.onReady(async (info) => {
    console.log('Email Fraud Detector v4.2.0 (Phase 2 Silent) script loaded, host:', info.host);
    if (info.host === Office.HostType.Outlook) {
        console.log('Email Fraud Detector v4.2.0 initializing for Outlook...');
        await initializeMsal();
        setupEventHandlers();
        analyzeCurrentEmail();
        setupAutoScan();
        console.log('Email Fraud Detector v4.2.0 ready');
    }
});
...
function displayResults(warnings) {
    ...
    if (warnings.length > 0) {
        const WARNING_PRIORITY = {
            ...
            'wire-fraud': 17,
            'phishing-urgency': 18,
            'phase2-phishing-pattern': 0  // v4.2.0: Phase 2 at top when active
        };
